...
class ExamRepository extends BaseRepository implements ExamInterface
{
    ...
    public function getAllByScheduleAndCategoryPaginatedWithParams($params, $scheduleId, $categoryId, $limit = 10)
    {
        $exams = $this->model;
...
//        ]);
//
//        dd($exams->where('schedule_id', $scheduleId)->get());

        if(isset($params->search) && !empty($params->search)) $exams->where('title', 'like', '%' . $params->search . '%');
        $exams = $exams->where('schedule_id', $scheduleId)->where('category_id', $categoryId)->with(['category', 'questionTitle', 'transaction'])->orderBy('created_at', 'ASC')->paginate($limit);
        return $exams;
    }
