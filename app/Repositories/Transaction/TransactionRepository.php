...
class TransactionRepository extends BaseRepository implements TransactionInterface
{
    ...
    public function getAllSimplePaginatedWithParams($params, $limit = 10)
    {
        ...
        if (isset($params->search) && !empty($params->search))
            $transactions->where('code', 'like', '%' . $params->search . '%');
    ...
    public function getAllPaginatedWithParams($params, $limit = 10)
    {
        ...
        $transactions = $transactions
            ->whereHas('exam') // pastikan exam masih ada
            ->whereHas('exam.category') // pastikan category exam masih ada
            ->with(['user', 'exam', 'exam.category'])
    ...
    public function getTransactionMonthly($limit = 10)
    {
        $transactions = $this->model
            ...
            ->whereHas('exam') // pastikan exam masih ada
            ->whereHas('exam.category') // pastikan category dari exam juga masih ada
            ->with(['user', 'exam', 'exam.category'])
    ...
    public function getSummaryTransactionByUser($limit = 5)
    {
        ...
        $transactions = $transaction->with([
            ...
            'exam.category'
        ])->where('user_id', Auth::user()->id)->orderBy('created_at', 'DESC')->limit($limit)->get();
