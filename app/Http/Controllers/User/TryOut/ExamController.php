...
class ExamController extends Controller
{
     ...
     */
    public function index(Request $request, $schedule, $category)
    {
        if (!$this->categoryService->find($category))
            return abort('404', 'uppss....');

        return inertia('User/TryOut/Exam/Index', [
            'exams' => $this->examService->getAllByScheduleAndCategoryPaginatedWithParams($request, $schedule, $category),
        ]);
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function show($category, $schedule, $exam)
    {
        //        dd([
//            $category,
//            $schedule,
...
//        ]);
        ...
        //        if(!$this->categoryService->find($category)) return abort('404', 'uppss....');
//        if(!$this->examService->find($exam)) return abort('404', 'uppss....');

        //get grade / nilai
        $grade = Grade::where('exam_id', $exam)
    ...
    public function examShowQuestion($id, $section, Request $request)
    {
        ...
        if ($grade->is_finished == 1) {
            return redirect()->route('user.schedules.categories.exams.show', [$exam->schedule_id, $grade->category_id, $grade->exam_id]);
        }
    ...
    {
        if (!$this->examService->find($examId))
            return abort('404', 'uppss....');
        ...
        //redirect hasil
        return redirect()->route('user.schedules.categories.exams.show', [$exam->schedule_id, $exam->category_id, $exam->id]);
    }
}