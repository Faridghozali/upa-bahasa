...
class TransactionController extends Controller
{
    ...
    {
//        dd($this->transactionService->find('549f1082-0aa8-44cb-b8bc-cb72cb0ec037'));
    ...
    public function buy(Request $request, $id)
    {
        try {
            ...
            if($this->transactionService->checkTransactionPendingByUser()) {
                return redirect()->route('user.schedules.categories.exams.index', [
                    'schedule' => $exam->schedule_id,
                    'category'  => $exam->category_id,
                        'exam' => $exam->id
                    ]
                )->with('error', 'Anda memiliki transaksi yang terpending, silakan selesaikan terlebih dahulu transaksi tersebut atau silakan hubungi admin');
            }
